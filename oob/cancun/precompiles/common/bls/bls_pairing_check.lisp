...
(defun (prc-blspairingcheck---precompile-cost_PRC_BLS_PAIRING_CHECK_PAIR_SIZE)       (*    (prc-blspairingcheck---cds-is-multiple-of-min-bls-pairing-check-size)
...
(defconstraint prc-blspairingcheck---mod-cds-by-PRC_BLS_PAIRING_CHECK_PAIR_SIZE (:guard (* (assumption---fresh-new-stamp) (prc-blspairingcheck---standard-precondition)))
  (call-to-MOD 2 0 (prc---cds) 0 PRC_BLS_PAIRING_CHECK_PAIR_SIZE))
...
(defconstraint prc-blspairingcheck---compare-call-gas-against-precompile-cost (:guard (* (assumption---fresh-new-stamp) (prc-blspairingcheck---standard-precondition)))
  (if-zero (prc-blspairingcheck---cds-is-multiple-of-min-bls-pairing-check-size)
           ...
           (begin (vanishes! (shift ADD_FLAG 4))
                  ...
                  (eq! (* (shift [OUTGOING_DATA 4] 4) PRC_BLS_PAIRING_CHECK_PAIR_SIZE)
                       (prc-blspairingcheck---precompile-cost_PRC_BLS_PAIRING_CHECK_PAIR_SIZE)))))
  ...
  (begin (eq! (prc---hub-success)
         ...
         (if-zero (prc---hub-success)
                  ...
                  (eq! (* (prc---return-gas) PRC_BLS_PAIRING_CHECK_PAIR_SIZE)
                       (- (* (prc---callee-gas) PRC_BLS_PAIRING_CHECK_PAIR_SIZE) (prc-blspairingcheck---precompile-cost_PRC_BLS_PAIRING_CHECK_PAIR_SIZE))))))
