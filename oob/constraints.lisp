...
(defun (lookup_sum)
  (+ ADD_FLAG MOD_FLAG WCP_FLAG))

(defun (wght_lookup_sum)
  (+ (* 1 ADD_FLAG) (* 2 MOD_FLAG) (* 3 WCP_FLAG)))

;; TODO: parametric versions, shall we generalize this and callToX as in the specs? What about noCall?
;; (defun (lookup_sum k)
...
(defconstraint valid-rdc-future (:guard (* (standing-hypothesis) (rdc-hypothesis)))
  (begin (eq! (next ADD_FLAG) OUTGOING_RES_LO)
         ...
         (if-not-zero OUTGOING_RES_LO
                      (begin (eq! (next OUTGOING_INST) ADD)
...
(defconstraint valid-rdc-future-future (:guard (* (standing-hypothesis) (rdc-hypothesis)))
  (begin (vanishes! (shift ADD_FLAG 2))
         ...
         (if-not-zero OUTGOING_RES_LO
                      (begin (eq! (shift OUTGOING_INST 2) GT)
...
(defconstraint valid-prc-ecpairing-future-future-future-future (:guard (* (standing-hypothesis) (prc-hypothesis) (prc-ecpairing-hypothesis)))
  (begin (vanishes! (shift ADD_FLAG 4))
         ...
         (if-not-zero (prc-ecpairing___is_multiple_192)
                      (begin (eq! (shift OUTGOING_INST 4) LT)
