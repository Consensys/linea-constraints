...
(defun (modexp-running)(* MODEXP CALL_DATA_SIZE))

(defconstraint execution-modexp-1 (:guard (modexp-running))
  (if-not-zero COUNTER
               (begin (eq! (shift WCP_ARG1_LO 2)
                           (shift MODEXP_PARAMS 1))
                      (eq! (shift WCP_ARG2_LO 2)
                           (shift MODEXP_PARAMS 5)))))
...
(defconstraint execution-modexp-4 (:guard (modexp-running))
  (if-zero COUNTER
           (if-zero-else (shift MODEXP_PARAMS 6)
                         (eq! (shift MODEXP_BYTE_ACC 15)
                              (shift MODEXP_PARAMS 7))
                         (eq! (shift MODEXP_BYTE_ACC 15)
                              (shift MODEXP_PARAMS 6)))))
  

(defconstraint execution-modexp-5 (:guard (modexp-running))
  (if-zero-else MODEXP_BYTE_ACC
                (vanishes! MODEXP_BYTE_SWITCH)
                (eq! MODEXP_BYTE_SWITCH 1)))
         ...
         (if-zero MODEXP_BYTE_SWITCH
                  (if-eq-else COUNTER 15
                              ...
                              (if-eq (next MODEXP_BYTE_SWITCH) 1
                                     (eq! MODEXP_E_LEADING_WORD_FIRST_BYTE
                                          (next MODEXP_BYTES)))))))
  ...
  (begin (if-zero-else COUNTER
                       ...
                       (eq! MODEXP_BIT_ACC
                            (+ (* 2 (prev MODEXP_BIT_ACC))
                               MODEXP_BITS)))
         (if-eq COUNTER 15
                (eq! MODEXP_BIT_ACC MODEXP_E_LEADING_WORD_FIRST_BYTE))))
  

(defconstraint execution-modexp-9 (:guard (modexp-running))
  (if-zero-else MODEXP_BIT_ACC
                (vanishes! MODEXP_BIT_SWITCH)
                (eq! MODEXP_BIT_SWITCH 1)))
...
(defconstraint execution-modexp-12 (:guard (modexp-running))
  (if-zero-else MODEXP_E_LEADING_WORD_BIT_LENGTH
                ...
                (eq! MODEXP_E_LEADING_WORD_LOG
                     (- MODEXP_E_LEADING_WORD_BIT_LENGTH 1))))
  

(defconstraint execution-modexp-13 (:guard (modexp-running))
  (if-zero COUNTER
           (begin (eq! (shift WCP_ARG1_LO 3) 32)
                  (eq! (shift WCP_ARG2_LO 3)
                       (shift MODEXP_PARAMS 3)))))
...
(defconstraint execution-modexp-16 (:guard (modexp-running))
  (if-zero COUNTER
           (begin (eq! (shift MOD_ARG1_LO 2)
                       (* MOD_RES MOD_RES MODEXP_MAX_LE_PRIME_AND_1))
...
(defconstraint no-return-data-if-failure ()
  (if-zero SUCCESS (vanishes! RETURN_DATA_SIZE)))
...
(defconstraint provides-return-data ()
  (if-zero-else RETURN_DATA_SIZE
                (vanishes! PROVIDES_RETURN_DATA)
                (eq! PROVIDES_RETURN_DATA 1)))
