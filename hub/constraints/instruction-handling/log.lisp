...
;;                                             ;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defun (log-inst-instruction)
  stack/INSTRUCTION)

(defun (log-inst-offset-hi)
  [stack/STACK_ITEM_VALUE_HI 1])

(defun (log-inst-offset-lo)
  [stack/STACK_ITEM_VALUE_LO 1])

(defun (log-inst-size-hi)
  [stack/STACK_ITEM_VALUE_HI 2])

(defun (log-inst-size-lo)
  [stack/STACK_ITEM_VALUE_LO 2])

(defun (log-inst-standard-hypothesis)
  (* PEEK_AT_STACK stack/LOG_FLAG (- 1 stack/SUX stack/SOX) (- 1 COUNTER_TLI)))
...
(defconstraint log-inst-setting-the-stack-pattern (:guard (log-inst-standard-hypothesis))
  (log-stack-pattern (- stack/INSTRUCTION EVM_INST_LOG0) [stack/DEC_FLAG 1] [stack/DEC_FLAG 2] [stack/DEC_FLAG 3] [stack/DEC_FLAG 4]))

(defconstraint log-inst-allowable-exceptions (:guard (log-inst-standard-hypothesis));; TODO: solo debug constraint plz
  (begin (vanishes! 0)
         (debug (eq! XAHOY (+ stack/STATICX stack/MXPX stack/OOGX)))))
...
(defconstraint log-inst-setting-the-peeking-flags (:guard (log-inst-standard-hypothesis))
  (if-zero (force-bin stack/STATICX)
           ;; STATICX = 0
           (eq! NSR
                (+ (shift PEEK_AT_CONTEXT log-context-row-offset)
...
(defconstraint log-inst-justifying-static-exception (:guard (log-inst-standard-hypothesis))
  (begin (read-context-data 2 CONTEXT_NUMBER)
...
(defconstraint log-inst-justifying-memory-expansion-exception (:guard (log-inst-standard-hypothesis))
  (if-zero (force-bin stack/STATICX)
           (eq! stack/MXPX (shift misc/MXP_MXPX log-misc-row-offset))))

(defconstraint log-inst-setting-the-gas-cost (:guard (log-inst-standard-hypothesis))
  (if-zero (force-bin (+ stack/STATICX stack/MXPX))
           (eq! GAS_COST
                (+ stack/STATIC_GAS (shift misc/MXP_GAS_MXP log-misc-row-offset)))
           (eq! GAS_COST 0)))

(defconstraint log-inst-the-final-context-row (:guard (log-inst-standard-hypothesis))
  (begin (if-not-zero stack/STATICX
                      (execution-provides-empty-return-data log-staticx-context-row-offset))
         (if-not-zero (+ stack/MXPX stack/OOGX)
                      (execution-provides-empty-return-data log-other-x-context-row-offset))))
...
(defconstraint log-inst-setting-MISC-module-flags (:guard (log-inst-standard-hypothesis))
  (eq! (weighted-MISC-flag-sum log-misc-row-offset)
       (+ (* MISC_WEIGHT_MMU (trigger_MMU)) MISC_WEIGHT_MXP)))
...
(defun (trigger_MMU)
  (* (- 1 CONTEXT_WILL_REVERT) (shift misc/MXP_MTNTOP log-misc-row-offset)))
