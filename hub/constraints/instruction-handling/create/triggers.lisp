...
(defun    (trigger_MXP)        (+    (create-instruction---STACK-mxpx)
                                     (create-instruction---STACK-oogx)
                                     (scenario-shorthand-CREATE-unexceptional)
                                     ))
(defun    (trigger_STP)        (+    (create-instruction---STACK-oogx)
                                     (scenario-shorthand-CREATE-unexceptional)
                                     ))
(defun    (trigger_OOB)        (+    (scenario-shorthand-CREATE-unexceptional)))
(defun    (trigger_MMU)        (+    (create-instruction---hash-init-code)
                                     (create-instruction---hash-init-code-and-send-to-ROM)
                                     (create-instruction---send-init-code-to-ROM)
                                     ))
(defun    (trigger_HASHINFO)   (*    (create-instruction---is-CREATE2)
...
(defun    (trigger_RLPADDR)    (scenario-shorthand-CREATE-compute-deployment-address))
(defun    (trigger_ROMLEX)     (scenario-shorthand-CREATE-not-rebuffed-nonempty-init-code))


;; auxiliary shorthands required for (trigger_MMU)
