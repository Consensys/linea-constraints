...
(defconst
  ROFF_REVERT_INSTRUCTION___MISCELLANEOUS_ROW              1
  ;;
  ROFF_REVERT_INSTRUCTION___XAHOY_CALLER_CONTEXT_ROW       2
  ;;
  ROFF_REVERT_INSTRUCTION___NO_XAHOY_CURRENT_CONTEXT_ROW   2
  ROFF_REVERT_INSTRUCTION___NO_XAHOY_CALLER_CONTEXT_ROW    3)
...
(defun  (revert-instruction---MXP-memory-expansion-gas)            (shift   misc/MXP_GAS_MXP                  ROFF_REVERT_INSTRUCTION___MISCELLANEOUS_ROW))
(defun  (revert-instruction---MXP-size-1-is-nonzero-and-no-mxpx)   (shift   misc/MXP_SIZE_1_NONZERO_NO_MXPX   ROFF_REVERT_INSTRUCTION___MISCELLANEOUS_ROW))
(defun  (revert-instruction---current-context-is-root)             (shift   context/IS_ROOT                   ROFF_REVERT_INSTRUCTION___NO_XAHOY_CURRENT_CONTEXT_ROW))
(defun  (revert-instruction---r@o)                                 (shift   context/RETURN_AT_OFFSET          ROFF_REVERT_INSTRUCTION___NO_XAHOY_CURRENT_CONTEXT_ROW))
(defun  (revert-instruction---r@c)                                 (shift   context/RETURN_AT_CAPACITY        ROFF_REVERT_INSTRUCTION___NO_XAHOY_CURRENT_CONTEXT_ROW))
...
(defconstraint  revert-instruction---setting-the-peeking-flags                     (:guard (revert-instruction---standard-precondition))
                (if-not-zero  XAHOY
                              ;; XAHOY ≡ 1
                              (eq!  NSR
                                    (+  (shift  PEEK_AT_MISCELLANEOUS    ROFF_REVERT_INSTRUCTION___MISCELLANEOUS_ROW            )
                                        (shift  PEEK_AT_CONTEXT          ROFF_REVERT_INSTRUCTION___XAHOY_CALLER_CONTEXT_ROW     )))
                              ;; XAHOY ≡ 0
                              (eq! NSR
                                   (+  (shift  PEEK_AT_MISCELLANEOUS   ROFF_REVERT_INSTRUCTION___MISCELLANEOUS_ROW            )
                                       (shift  PEEK_AT_CONTEXT         ROFF_REVERT_INSTRUCTION___NO_XAHOY_CURRENT_CONTEXT_ROW )
                                       (shift  PEEK_AT_CONTEXT         ROFF_REVERT_INSTRUCTION___NO_XAHOY_CALLER_CONTEXT_ROW  )))))

(defconstraint  revert-instruction---setting-the-context-rows                      (:guard (revert-instruction---standard-precondition))
                (if-not-zero  XAHOY
                              ;; XAHOY ≡ 1
                              (execution-provides-empty-return-data      ROFF_REVERT_INSTRUCTION___XAHOY_CALLER_CONTEXT_ROW)
                              ;; XAHOY ≡ 0
                              (begin
                                (read-context-data   ROFF_REVERT_INSTRUCTION___NO_XAHOY_CURRENT_CONTEXT_ROW
                                ...
                                (if-not-zero   (force-bin (revert-instruction---current-context-is-root))
                                               ...
                                               (read-context-data    ROFF_REVERT_INSTRUCTION___NO_XAHOY_CALLER_CONTEXT_ROW
                                                                     (revert-instruction---caller-context)))
                                ;; current context ISN'T root
                                (provide-return-data   ROFF_REVERT_INSTRUCTION___NO_XAHOY_CALLER_CONTEXT_ROW         ;; row offset
...
(defconstraint  revert-instruction---setting-the-miscellaneous-row-module-flags    (:guard (revert-instruction---standard-precondition))
                (eq!  (weighted-MISC-flag-sum  ROFF_REVERT_INSTRUCTION___MISCELLANEOUS_ROW)
                      (+  MISC_WEIGHT_MXP
...
(defconstraint  revert-instruction---setting-the-MXP-data                          (:guard (revert-instruction---standard-precondition))
                (set-MXP-instruction-type-4 ROFF_REVERT_INSTRUCTION___MISCELLANEOUS_ROW   ;; row offset kappa
...
(defconstraint  revert-instruction---setting-the-MXPX                              (:guard (revert-instruction---standard-precondition))
                (eq!  stack/MXPX  (shift  misc/MXP_MXPX  ROFF_REVERT_INSTRUCTION___MISCELLANEOUS_ROW)))

(defconstraint  revert-instruction---setting-the-MMU-data                          (:guard (revert-instruction---standard-precondition))
                (if-not-zero  (shift  misc/MMU_FLAG  ROFF_REVERT_INSTRUCTION___MISCELLANEOUS_ROW)
                              (set-MMU-instruction---ram-to-ram-sans-padding    ROFF_REVERT_INSTRUCTION___MISCELLANEOUS_ROW  ;; row offset
