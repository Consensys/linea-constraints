...
(defun (tx-skip-wei-cost-for-sender)
  (+ (shift transaction/VALUE TX_SKIP_TRANSACTION_ROW_OFFSET)
...
(defun (tx-skip-is-deployment)
  (force-bin (shift transaction/IS_DEPLOYMENT TX_SKIP_TRANSACTION_ROW_OFFSET)))

;; sender account operation
;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defconstraint tx-skip-setting-sender-account-row (:guard (tx-skip-precondition))
  (begin (eq! (shift account/ADDRESS_HI TX_SKIP_SENDER_ACCOUNT_ROW_OFFSET)
              (shift transaction/FROM_ADDRESS_HI TX_SKIP_TRANSACTION_ROW_OFFSET))
         (eq! (shift account/ADDRESS_LO TX_SKIP_SENDER_ACCOUNT_ROW_OFFSET)
              (shift transaction/FROM_ADDRESS_LO TX_SKIP_TRANSACTION_ROW_OFFSET))
         ...
         (DOM-SUB-stamps---standard TX_SKIP_SENDER_ACCOUNT_ROW_OFFSET 0)))

;; recipient account operation
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defconstraint tx-skip-setting-recipient-account-row (:guard (tx-skip-precondition))
  (begin (eq! (shift account/ADDRESS_HI TX_SKIP_RECIPIENT_ACCOUNT_ROW_OFFSET)
              (shift transaction/TO_ADDRESS_HI TX_SKIP_TRANSACTION_ROW_OFFSET))
         (eq! (shift account/ADDRESS_LO TX_SKIP_RECIPIENT_ACCOUNT_ROW_OFFSET)
              (shift transaction/TO_ADDRESS_LO TX_SKIP_TRANSACTION_ROW_OFFSET))
         (account-increment-balance-by TX_SKIP_RECIPIENT_ACCOUNT_ROW_OFFSET
                                       (shift transaction/VALUE TX_SKIP_TRANSACTION_ROW_OFFSET))
         ...
         (DOM-SUB-stamps---standard TX_SKIP_RECIPIENT_ACCOUNT_ROW_OFFSET 1)))
...
(defconstraint tx-skip-setting-recipient-account-row-nonce-code-and-deployment-status-for-trivial-message-calls (:guard (tx-skip-precondition))
  (if-zero (tx-skip-is-deployment)
           ...
           (begin (account-same-nonce TX_SKIP_RECIPIENT_ACCOUNT_ROW_OFFSET)
                  (account-same-code TX_SKIP_RECIPIENT_ACCOUNT_ROW_OFFSET)
                  (account-same-deployment-number-and-status TX_SKIP_RECIPIENT_ACCOUNT_ROW_OFFSET))))
...
(defconstraint tx-skip-setting-recipient-account-row-code-for-trivial-deployments (:guard (tx-skip-precondition))
  (if-not-zero (tx-skip-is-deployment)
               ...
               (begin  ;; code
                      ...
                      (debug (eq! (shift account/CODE_HASH_HI TX_SKIP_RECIPIENT_ACCOUNT_ROW_OFFSET)
                                  EMPTY_KECCAK_HI))
                      (debug (eq! (shift account/CODE_HASH_LO TX_SKIP_RECIPIENT_ACCOUNT_ROW_OFFSET)
                                  EMPTY_KECCAK_LO))
...
(defconstraint tx-skip-recipient-is-no-precompile (:guard (tx-skip-precondition))
  (if-not-zero (tx-skip-is-deployment)
               ;; deployment ≡ 1 i.e. trivial deployments
               (begin (account-trim-address TX_SKIP_RECIPIENT_ACCOUNT_ROW_OFFSET
                                            (shift transaction/TO_ADDRESS_HI TX_SKIP_TRANSACTION_ROW_OFFSET)
                                            (shift transaction/TO_ADDRESS_LO TX_SKIP_TRANSACTION_ROW_OFFSET))
                      (account-isnt-precompile TX_SKIP_RECIPIENT_ACCOUNT_ROW_OFFSET))))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defun (coinbase-fee)
  (* (shift transaction/PRIORITY_FEE_PER_GAS TX_SKIP_TRANSACTION_ROW_OFFSET)
     (- (shift transaction/GAS_LIMIT TX_SKIP_TRANSACTION_ROW_OFFSET)
...
(defconstraint tx-skip-setting-coinbase-account-row (:guard (tx-skip-precondition))
  (begin (eq! (shift account/ADDRESS_HI TX_SKIP_COINBASE_ACCOUNT_ROW_OFFSET)
              (shift transaction/COINBASE_ADDRESS_HI TX_SKIP_TRANSACTION_ROW_OFFSET))
         (eq! (shift account/ADDRESS_LO TX_SKIP_COINBASE_ACCOUNT_ROW_OFFSET)
              (shift transaction/COINBASE_ADDRESS_LO TX_SKIP_TRANSACTION_ROW_OFFSET))
         ...
         (DOM-SUB-stamps---standard TX_SKIP_COINBASE_ACCOUNT_ROW_OFFSET 2)))

(defconstraint tx-skip-transaction-row-partially-justifying-requires-evm-execution (:guard (tx-skip-precondition))
  (begin (vanishes! (shift transaction/REQUIRES_EVM_EXECUTION TX_SKIP_TRANSACTION_ROW_OFFSET))
...
(defconstraint tx-skip-transaction-row-justifying-status-code (:guard (tx-skip-precondition))
  (eq! (shift transaction/STATUS_CODE TX_SKIP_TRANSACTION_ROW_OFFSET) 1))
